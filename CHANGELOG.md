...
- `refresh_and_wait()`: 全連携口座の更新実行とデータ反映待ちをまとめて行う
- `wait_refresh(baseline=...)`: 「登録日（最終取得日）の前進」を完了条件にできるように。
  「更新状態」列はデータ取得完了後も長時間「更新中」のまま残ることを観測したため
  (データは実測 15〜46 秒で反映。docs/specs/accounts/refresh.md 参照)、
  更新実行を伴う待機ではこちらを推奨。`account refresh --wait` もこの方式に変更

## v0.3.1 (2026-07-11)

### 変更

- `wait_refresh()`: ポーリング毎の「更新待ち」INFO ログを、更新中口座の顔ぶれが
  変わったときだけに削減(変化なしは DEBUG)。件数も併記
